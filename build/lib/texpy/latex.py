...
def principale(n,nrif=None,nult=None):
	if n==0: return ["0",0]
	temp=nrif
	if nrif is None:nrif=n
	if nult is None:
		if absolute(nrif)==absolute(n):
			if temp is None:nult=n/100 #ns_tex(456,456)=4.56 x 10^2
			else: nult=n
...
#Author:Francesco Sacco
def ne_stringa(x,dx=None,unit=None):
	if dx is None: return ns(x,unit=unit)    
	if dx>absolute(x): return 0,ns(dx,nult=dx,unit=unit)
	return ns(x,nult=dx,unit=unit), ns(dx,x,unit=unit)
nes=vectorize(ne_stringa)
...
def mat(Matrice,file=None):
	if (Matrice.ndmin>2):
		print('Non è possibile stampare "Matrici" con dimensione >2')
		return	
